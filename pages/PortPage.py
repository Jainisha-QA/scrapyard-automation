...
class PortPage(BasePage):

    ADD_BTN = (By.XPATH, "//button[contains(text(),'Add')]")
    DROP_DOWN = (By.XPATH, "//span[contains(text(),'Select Port')]")
    OPTION = (By.XPATH, "//span[contains(text(), 'Dry Port')]")
    INPUT_TEXTBOX1 = (By.XPATH, "//input[@placeholder='Enter port name']")
    ...
    def __init__(self, driver):
        ...
        if self.driver.current_url == TestData.PORTS_URL:
            pass
        else:
            self.driver.get(TestData.PORTS_URL)
    ...
    def check_require_msg(self):
        ...
        msg2 = self.below_element_text(self.DROP_DOWN, self.REQUIRE_MSG[1])
