...
class SupGupPage(BasePage):
    ADD_BTN = (By.XPATH, "//button[contains(text(),'Add')]")
    DROP_DOWN = (By.XPATH, "//span[contains(text(),'Select Country')]")
    OPTION = (By.XPATH, "//span[contains(text(), 'Afghanistan')]")
    INPUT_TEXTBOX = (By.XPATH, "//input[@placeholder='Enter Supplier Group']")
    ...
    def __init__(self, driver):
        ...
        if self.driver.current_url == TestData.SUPGROUP_URL:
            pass
        else:
            self.driver.get(TestData.SUPGROUP_URL)
    ...
    def check_require_msg(self):
        ...
        self.action_click(self.DROP_DOWN)
        time.sleep(2)
        self.action_click(self.DROP_DOWN)
        msg0 = self.below_element_text(self.INPUT_TEXTBOX, self.REQUIRE_MSG[1])
        msg1 = self.below_element_text(self.DROP_DOWN, self.REQUIRE_MSG[1])
        return msg0, msg1
